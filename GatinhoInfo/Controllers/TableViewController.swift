...
class TableViewController: UIViewController {
    ...
    public lazy var tableView: UITableView = {
      ...
      table.register(TableViewCell.self, forCellReuseIdentifier: StringsEnum.AnimalCellIdentifier.rawValue.localize())
        table.delegate = self
        return table
    }()
...
extension TableViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
      guard let cell = tableView.dequeueReusableCell(withIdentifier: StringsEnum.AnimalCellIdentifier.rawValue.localize(), for: indexPath) as? TableViewCell else { fatalError() }
